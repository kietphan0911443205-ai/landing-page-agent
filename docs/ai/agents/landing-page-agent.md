...
- Xác thực OAuth hoặc kết nối hosting khi hệ thống yêu cầu; với Stitch, khách hàng chỉ xác thực OAuth trong giao diện Codex và agent tự tạo project mới theo customer job; credential phải đi qua secret/environment manager. Không yêu cầu khách hàng chạy CLI, cài connector, sửa config hoặc gửi project link. Nếu Stitch MCP chưa khả dụng, agent phải báo blocker và dừng, không âm thầm thay thế bằng HTML/CSS/JavaScript.
