---
name: aibos-ui-ux
description: "Thiết kế, xây dựng và review UI/UX cho AIBOS bằng cơ sở dữ liệu UI/UX Pro Max; dùng cho page, component, màu sắc, typography, layout, accessibility, animation hoặc data visualization. Output tài liệu và nhận xét bằng tiếng Việt."
---

# AIBOS UI/UX — Design Intelligence
...
Skill này tích hợp đầy đủ UI/UX Pro Max ở dạng portable, gồm searchable design intelligence, design-system generator, style/color/typography/product/UX/motion/chart/icon data và stack guidelines. Toàn bộ tài liệu, đề xuất, review và design system phải được viết bằng tiếng Việt; giữ nguyên tên kỹ thuật, tên token, API, component và code khi cần.

## Quy tắc riêng của AIBOS

- Đọc `DESIGN.md` ở thư mục gốc trước khi thay đổi UI.
- `ui/src/index.css` là nguồn token duy nhất. Không thêm màu, spacing, radius, typography, shadow hoặc motion dạng hardcode vào component/page.
- Tuân thủ `pnpm check:token-gates` sau thay đổi UI.
- Giữ định hướng sản phẩm control plane: ưu tiên khả năng quét nhanh trạng thái, nhận biết việc cần xử lý và thực hiện hành động tiếp theo.
- Dùng thuật ngữ nhất quán của sản phẩm; không tự ý đổi tên domain concept hoặc copy hiện có nếu task không yêu cầu.
- Khi review, phân biệt rõ lỗi vi phạm quy tắc, rủi ro UX và đề xuất cải tiến.

## Skill bổ trợ cho UI/UX Designer Agent

Bộ skill bổ trợ đã được đưa vào `related-skills/`. Đọc [related-skills.md](references/related-skills.md) để chọn đúng skill theo vấn đề; không nạp toàn bộ nếu task chỉ cần một nhóm nhỏ. Các skill bổ trợ gồm:

- Discovery/UX: `aibos-ui-ux-lean-ux`, `aibos-ui-ux-design-everyday-things`, `aibos-ui-ux-hooked-ux`.
- Setup/Onboarding: `aibos-ui-ux-setup-flow` for activation, multi-step setup, progress, resume and recovery.
- Visual Design: `aibos-ui-ux-refactoring-ui`, `aibos-ui-ux-web-typography`, `aibos-ui-ux-top-design`.
- Interaction: `aibos-ui-ux-microinteractions`.
- Platform: `aibos-ui-ux-ios-hig-design`.
- Review/QA: `aibos-ui-ux-heuristics`, `aibos-ui-ux-steve-jobs-review`, `aibos-ui-ux-improve-retention`.

Khi task yêu cầu UI/UX Designer xử lý end-to-end, dùng thứ tự: `ui-ux-pro-max` → skill chuyên môn phù hợp → review bằng `aibos-ui-ux-heuristics` và kiểm tra trực quan theo quy tắc AIBOS.
...
The search script lives inside this skill's own directory. Invoke it through the path resolved from the installed skill directory:
...
```bash
python3 skills/aibos-ui-ux/scripts/search.py "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python3 skills/aibos-ui-ux/scripts/search.py "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python3 skills/aibos-ui-ux/scripts/search.py "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python3 skills/aibos-ui-ux/scripts/search.py "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python3 skills/aibos-ui-ux/scripts/search.py "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python3 skills/aibos-ui-ux/scripts/search.py "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python3 skills/aibos-ui-ux/scripts/search.py "<keyword>" --stack <stack>
```
...
python3 skills/aibos-ui-ux/scripts/search.py "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python3 skills/aibos-ui-ux/scripts/search.py "search loading animation" --domain ux

# Step 4: stack guidelines
python3 skills/aibos-ui-ux/scripts/search.py "suspense streaming bundle" --stack nextjs
```
